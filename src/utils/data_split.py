...
def link_class_split(data: torch_geometric.data.Data, size: int = None, splits: int = 2, prob_test: float = 0.15,
    ...
    Return types:
        * **datasets** - A dict include training/validation/testing splits of edges and labels. For split index i:
            ...
            * datasets[i]['train'/'val'/'testing']['label'] (List): the labels of edges:
                ...
                * If task == "sign": 0 (negative edge), 1 (positive edge),2(no edge)This is the link sign prediction task for signed networks.
    """
    assert task in ["existence", "direction", "three_class_digraph", "four_class_signed_digraph",
    ...
    # Strict directed negative sampling:
    # sample from all ordered node pairs (i, j) with i != j and A[i, j] == 0.
    # If A->B exists but B->A does not, then (B, A) can be sampled as a no-edge pair.
    num_neg_samples = len(edge_index.T)
    existing_edge_set = set(
        (int(u), int(v)) for u, v in edge_index.T.tolist() if int(u) != int(v)
    )
    max_possible_neg = size * (size - 1) - len(existing_edge_set)

    if num_neg_samples > max_possible_neg:
        raise ValueError(
            f"There are not enough strict directed negative edges: "
            f"need={num_neg_samples}, available={max_possible_neg}."
        )

    neg_edges = []
    forbidden_pairs = set(existing_edge_set)
    neg_rs = np.random.RandomState(seed)
    trials = 0
    max_trials = max(10000, num_neg_samples * 100)

    while len(neg_edges) < num_neg_samples and trials < max_trials:
        batch_size = max(4096, (num_neg_samples - len(neg_edges)) * 4)
        src = neg_rs.randint(0, size, size=batch_size)
        dst = neg_rs.randint(0, size, size=batch_size)
    ...
    if len(neg_edges) < num_neg_samples:
        ...
        need = num_neg_samples - len(neg_edges)
        if len(remaining) < need:
            raise ValueError(
                f"There are not enough strict directed negative edges: "
                f"need={num_neg_samples}, got={len(neg_edges) + len(remaining)}."
            )

        neg_rs.shuffle(remaining)
    ...
    if maintain_connect:
        ...
        mst_undir = list(tree.minimum_spanning_edges(
    ...
    for ind in range(splits):
        ...
        if task in ["direction", 'three_class_digraph']:
            ...
            nmst = np.array(nmst, dtype=np.int64)
            ...
            ids_test = np.array(
                test_pos_edges +
                test_neg_edges +
                neg_edges[:test_noedge_num],
                dtype=np.int64
            )

            neg_cursor = test_noedge_num
            ...
            ids_val = np.array(
                val_pos_edges +
                val_neg_edges +
                neg_edges[neg_cursor:neg_cursor + val_noedge_num],
                dtype=np.int64
            )

            neg_cursor += val_noedge_num
            ...
            ids_train = np.array(
                train_true_edges +
                neg_edges[neg_cursor:neg_cursor + train_noedge_num],
                dtype=np.int64
            )
            ...
            for part_name, labels_part in [
                ("train", labels_train),
                ("val", labels_val),
                ("test", labels_test)
            ]:
                count_part = np.bincount(labels_part.astype(np.int64), minlength=3)

                n_neg_reg = int(count_part[0])
                n_pos_reg = int(count_part[1])
                n_no_edge = int(count_part[2])
